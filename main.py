...
def load_vgg(sess, vgg_path):
    ...
    # load the vgg model graph into the session

    tf.saved_model.loader.load(sess, [vgg_tag], vgg_path)

    # get vgg layer weights from the current graph
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # reshape the logits and the labels
    ...
    # use Adam optimizer to minimize the cross-entropy loss
...
def run():
    ...
    with tf.Session() as sess:
        ...
        nn_last_layer = layers(vgg_layer3_out=layer3_out, vgg_layer4_out=layer4_out, vgg_layer7_out=layer7_out, num_classes=num_classes)
        ...
        train_nn(sess=sess, epochs=epochs, batch_size=batch_size, get_batches_fn=get_batches_fn, train_op=train_op, cross_entropy_loss=cross_entropy_loss,
